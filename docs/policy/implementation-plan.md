...
- 검증 결과:
  - `dart analyze` 통과
  - `flutter test --no-pub` 전체 18개 테스트 통과
...
- [x] `flutter test --no-pub` 전체 18개 테스트가 통과한다.
- [x] 320×568 테스트 화면에서 주요 탭에 overflow가 없는지 확인한다.
- [ ] 긴 정책명과 긴 구·군명이 줄바꿈 또는 ellipsis로 안전하게 표시되는지 확인한다.
- [ ] 정책 탭을 이동했다 돌아와도 예상한 화면 상태가 유지되는지 확인한다.

### 단계 7. PR 준비

- [ ] 변경 범위가 정책 기능과 필요한 최소 라우트 파일에 한정됐는지 확인한다.
- [ ] API 호출이나 저장 기능이 포함되지 않았는지 확인한다.
- [ ] 이슈 번호와 테스트 결과를 PR 본문에 기록한다.
...
- [ ] #8의 조건 입력 항목과 검증 상태가 모두 구현됐다.
- [ ] #9의 목록·상세·외부 이동 확인 흐름이 연결됐다.
- [ ] 실제 API, 저장, 신청 기능처럼 오해할 표현이 없다.
- [ ] nullable 지원금과 마감일을 안전하게 표시한다.
- [ ] `flutter analyze`와 `flutter test`가 통과한다.
- [ ] 다른 기능 폴더의 불필요한 변경이 없다.
